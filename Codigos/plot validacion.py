...
file_path = 'C:/Users/ferfo/OneDrive/Desktop/metricas-diarias.xlsx'
...
# Función para escalar el tamaño de las pelotitas según el valor de la métrica
def scale_sizes(values, s_min=20, s_max=200):
    scaled_sizes = s_min + (s_max - s_min) * (values - np.min(values)) / (np.max(values) - np.min(values))
    return scaled_sizes
...
for i, (metric_merra, metric_cams, title, cmap, clim, tick) in enumerate(zip(metrics_merra, metrics_cams, titles, cmaps, clims, ticks)):
    ...
    # Escalar tamaños para las métricas
    sizes_merra = scale_sizes(metric_merra)  # Tamaño para MERRA
    sizes_cams = scale_sizes(metric_cams)    # Tamaño para CAMS
    ...
    scatter_merra = ax.scatter(
        ...
        s=sizes_merra, edgecolor='k', transform=ccrs.PlateCarree()
    )
    ...
    scatter_cams = ax.scatter(
        ...
        s=sizes_cams, edgecolor='k', transform=ccrs.PlateCarree()
    )
...
# Guardar la figura
fig.savefig('C:/Users/ferfo/OneDrive/Desktop/comparacion_metricas_merra_cams_diario.png', dpi=300, bbox_inches='tight')
